...
クリエイターの返答は、実際に学習者に話しかけた生のセリフとして扱い、その口調・語尾・励まし方をそのまま読み取ってください。
...
以下のJSON形式のみで出力してください（説明文は不要）:
{
  ...
  "sample_reply": "学習者が「最近やる気が出なくて、続けられるか不安です…」と相談してきた場合に、このクリエイターが実際に返すであろう一言（このクリエイター自身の口調・語尾・励まし方をそのまま反映した1〜3文程度の生のセリフ。クリエイター紹介ページのサンプルチャットに表示される）"
}

回答に書かれていない項目があっても、文体・語尾・話の運び方から推測して必ず全項目を埋めてください。
sample_replyは絶対に「そう感じる時もありますよね」のような汎用的な定型文にせず、このクリエイター固有の口癖・語尾・距離感をはっきり反映させてください。
"""
