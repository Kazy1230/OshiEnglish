...
def _get_owned_idea(db: Session, idea_id: int, current_user) -> ContentIdea:
    idea = db.query(ContentIdea).filter(ContentIdea.id == idea_id).first()
    if not idea:
        raise HTTPException(status_code=404, detail="アイデアが見つかりません")
    if current_user.role != "admin":
        profile = db.query(CreatorProfile).filter(CreatorProfile.user_id == current_user.id).first()
        if not profile or idea.creator_id != profile.id:
            raise HTTPException(status_code=403, detail="このアイデアを操作する権限がありません")
    return idea
...
def _serialize_idea(idea: ContentIdea) -> dict:
    return {
        "id": idea.id,
        "text": idea.text,
...
def list_ideas(
    ...
    if creator_id is None:
        return []
    ideas = db.query(ContentIdea).filter(ContentIdea.creator_id == creator_id).order_by(ContentIdea.updated_at.desc()).all()
...
def create_idea(
    ...
    idea = ContentIdea(creator_id=creator_id, text=data.text.strip())
    db.add(idea)
    db.commit()
    db.refresh(idea)
...
def update_idea(
    ...
    if not data.text.strip():
        raise HTTPException(status_code=400, detail="内容を入力してください")
    idea.text = data.text.strip()
    db.commit()
    db.refresh(idea)
    return _serialize_idea(idea)


@router.delete("/idea-pool/{idea_id}")
def delete_idea(
    idea_id: int,
    current_user=Depends(get_current_creator_or_admin),
    db: Session = Depends(get_db),
