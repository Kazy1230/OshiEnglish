...
class Character(Base):
    ...
    # このキャラクター(人格)を所有するクリエイター。1クリエイター=1人格のため一意制約を持つ
    creator_id = Column(Integer, ForeignKey("creator_profiles.id"), nullable=True, unique=True, index=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    customers = relationship("Customer", back_populates="character")
    creator = relationship("CreatorProfile", back_populates="character", uselist=False)
