...
| SCR-17 | /creator/courses/[id]/calendar | 90日カレンダー編集 | クリエイター |
...
| エリア | 内容 |
|---|---|
| カレンダービュー | 90日分を月単位で表示。各日をクリックで編集パネル展開 |
...
| POST | /interview/generate-profile | 回答をもとに人格プロファイルを生成 | 要(creator) |
...
| POST | /courses | コース新規作成 | 要(creator) |
| PUT | /courses/[id] | コース更新 | 要(creator/本人) |
| DELETE | /courses/[id] | コース削除 | 要(creator/本人) |
| POST | /courses/[id]/generate-days | 90日コースをAI生成 | 要(creator/本人) |

#### 90日カレンダー (Course Days)
| メソッド | パス | 概要 | 認証 |
|---|---|---|---|
| GET | /courses/[id]/days | 90日分の日次コンテンツ一覧 | 要 |
