...
| 運営 | クリエイター審査、コース審査、通報対応、Tier B未回答監視、ユーザー管理、教材プリセット管理 | `/admin` |
...
flowchart TD
  ...
  B --> C{"承認?"}
  C -- "承認" --> D["creator_profile.statusをactiveに変更"]
  C -- "却下" --> E["creator_profile.statusをsuspendedに変更し、却下理由をメール通知（DBには保存しない）"]
  D --> F["コース作成権限付与"]
```
...
| 教材プリセット管理 | `/admin` の教材プリセットタブでプリセット教材（書籍/単語帳）の登録・編集・削除 |
...
| `/creator/courses/[id]/textbooks` | 教材設定 | 教材検索、コース教材追加、章/項目のDay割当、AIに使い方を相談してプランを自動生成・適用 | `GET /textbooks`, `GET/POST /courses/[id]/textbooks`, `PUT /course-textbooks/[id]`, `DELETE /course-textbooks/[id]`, `PUT /course-textbooks/[id]/day-assignments`, `POST /courses/[id]/textbooks/plan`, `POST /courses/[id]/textbooks/plan/apply` |
...
| 教材プリセット | プリセット教材（書籍/単語帳）一覧、新規登録、編集、削除（コースで使用中は削除不可） | `GET /admin/textbooks`, `POST /admin/textbooks`, `PUT /admin/textbooks/[id]`, `DELETE /admin/textbooks/[id]` |
