...
## 4. カリキュラム壁打ち用プロンプト生成（v2.0）

**ファイル:** `backend/app/routers/curriculum.py`（`get_curriculum_prompt` 関数内で生成）
**エンドポイント:** `GET /courses/{course_id}/curriculum-prompt`
**用途:** クリエイターが ChatGPT / Claude 等の外部AIに貼り付けて章立てを壁打ちするためのプロンプトを生成する。AIによる自動生成ではなく、クリエイターが手動でコピーして使う。
...
【講座の目的】{curriculum_purpose}
【対象者】{curriculum_target_audience}
【扱いたいトピック・要素】{curriculum_topics}
【期間感の目安】{curriculum_duration}
【講師としてのスタイル・こだわり】{curriculum_style}
【まだ迷っている・決めきれていない点】{curriculum_concerns}
【持っている動画】{curriculum_existing_videos}
```
...
## 4b. コース骨格生成 Layer 1（v1.x 方式・非推奨）

> **注意:** v2.0 では章/カード方式に移行したため、このプロンプトは現在使用されていない。v1.x の30日カレンダー方式の仕様として参考記録として残す。
...
## 5. タスクパーソナライズ Layer 2（v1.x 方式・非推奨）

> **注意:** v2.0 では章/カード方式に移行したため、このプロンプトは現在使用されていない。
...
## 9. コース品質チェック（v1.x 方式・非推奨）

> **注意:** v2.0 では章/カード方式に移行したため、このエンドポイントは現在使用されていない。
